...
const welcome = (
    ...
    try {
        ...
        ctx.fillText(`Hello, ${pushname.trim().length > 17 ? pushname.trim().substring(0, 16) + '...' : pushname.trim()}`, 430, 190)
        ctx.font = `36px Chakra Petch`
        ctx.fillText(`Welcome to the Group`, 430, 230)
        ctx.font = `bold 36px Chakra Petch`
        ctx.fillText(`${groupName.trim().length > 24 ? groupName.trim().substring(0, 23) + '...' : groupName.trim()}`, 430, 270)
        ctx.font = `30px Chakra Petch`
        ctx.fillStyle = (new Date()).getHours() >= 18 || (new Date()).getHours() <= 5 ? 'white' : 'black'
        ctx.fillText(`Don't forget to give your introduction`, 430, 340)
        ctx.fillText(`and follow the rules of the group!`, 430, 380)
